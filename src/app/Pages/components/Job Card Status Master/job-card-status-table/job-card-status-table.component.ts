...
export class JobCardStatusTableComponent {
  ...
  searchText: string = '';
  formTitle = "Manage Job Card Status";
  pageIndex = 1;
  pageSize = 10;
  ...
  search(reset: boolean = false) {
    ...
    this.api
      .getJobCardStatusData(this.pageIndex, this.pageSize, this.sortKey, sort, likeQuery)
      .subscribe(
        (data) => {
          if (data['code'] == 200) {
  ...
  add(): void {
    this.drawerTitle = 'Create New Job Card Status';
  ...
  edit(data: JobCardMasterData): void {
    this.drawerTitle = 'Update Job Card Status';

...
export class JobCardStatusDrawerComponent {
  ...
  save(addNew: boolean, JobCardStatusmaster: NgForm): void {
    ...
    if (this.isOk) {
      ...
      {
        if (this.data.ID) {
          this.api.updateJobCardStatus(this.data).subscribe((successCode: any) => {
            if (successCode.code == '200') {
              this.message.success('Job Card Updated Successfully', '');
              if (!addNew) this.drawerClose();
              this.isSpinning = false;
            } else {
              this.message.error('Job Card Not Updated', '');
              this.isSpinning = false;
            }
          });
        } else {
          this.api.createJobCardStatus(this.data).subscribe((successCode: any) => {
            if (successCode.code == '200') {
              this.message.success('Job Card Saved Successfully', '');
              if (!addNew) this.drawerClose();
              else {
                this.data = new JobCardMasterData();
              ...
              this.message.error('Job Card Not Saved', '');
              this.isSpinning = false;
            }
          });
        }
  ...
  onFileSelected(event: any) {
    ...
    if (
      ...
      this.message.error("Please Select Only Icon File", "");
      this.fileURL = null;
      this.isSpinning = false;
      this.progressBarImageOne = false;
      this.percentImageOne = 0;
  ...
  IconDeleteConfirm(data: any) {
    ...
    this.data.ICON = " ";
    this.fileURL = null;
    console.log("data icon", this.data.ICON);

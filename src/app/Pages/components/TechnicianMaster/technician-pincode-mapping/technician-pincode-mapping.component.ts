...
export class TechnicianPincodeMappingComponent {
  ...
  PincodeMapping() {
    ...
    this.api
      .getTechnicianPincodeMappedData(
        ...
        this.sortKey,
        sort,
        "AND ID =" + this.data.ID + this.filterQuery
      )
      .subscribe(
        (data) => {
          if (data["code"] === 200) {
  ...
  mapSelected() {
    ...
    this.api.addTechnicianPincodeMapping(this.data.ID, 1, dataToSave).subscribe(
      (successCode) => {
  ...
  onPincodeSelecttable(data: any, selected: boolean): void {
    ...
    // Update selected pincodes
    this.selectedPincode = this.PincodeMappingdata.filter(
      (item) => item.selected
    ).map((item) => item.ID);

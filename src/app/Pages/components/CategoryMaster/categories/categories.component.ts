...
export class CategoriesComponent {
  ...
  columns: string[][] = [ ["CATEGORY_NAME", "Category Name"] ]
  time = new Date()
  features = [];
  visible1 = false;
  ...
  this.loadingRecords = true;
    ...
    if (this.categoryName !== '') {
      likeQuery += (likeQuery ? ' AND ' : '') + `CATEGORY_NAME LIKE '%${this.categoryName.trim()}%'`;
    }
// Status Filter
if (this.statusFilter) {
  ...
  likeQuery += `IS_ACTIVE = ${this.statusFilter}`;
    ...
    this.api.getAllCategory(this.pageIndex, this.pageSize, this.sortKey, sort, likeQuery).subscribe(data => {
  ...
  add(): void {
    this.drawerTitle = "Create New Category";

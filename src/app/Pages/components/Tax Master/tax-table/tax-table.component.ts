 ...
 isValidInput(input: string): boolean {
  ...
  search(reset: boolean = false) {
    ...
    this.loadingRecords = true;
      ...
      // tax Filter
      if (this.taxtext !== '') {
        likeQuery += (likeQuery ? ' AND ' : '') + `NAME LIKE '%${this.taxtext.trim()}%'`;
      }

      // Short Code
      if (this.codetext !== '') {
        likeQuery += (likeQuery ? ' AND ' : '') + `SHORT_CODE LIKE '%${this.codetext.trim()}%'`;
      }
  ...
  add(): void {
    this.drawerTitle = 'Create New Tax ';
